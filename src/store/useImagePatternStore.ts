...
interface PatternState {
    patterns: IImagePattern[];
    loading: boolean;

    fetchPatterns: (uid: string) => Promise<void>;
    getPatternById: (uid: string, id: string) => Promise<IImagePattern | null>;
}

export const useImagePatternStore = create<PatternState>((set, get) => ({
    patterns: [],
    loading: false,
    ...
    fetchPatterns: async (uid: string) => {
        ...
        set({ loading: true });
        try {
            const q = query(collection(db, 'users', uid, 'ImagePatterns'), orderBy('createdAt', 'desc'));
    ...
    getPatternById: async (uid: string, id: string) => {
        ...
        try {
            ...
            if (docSnap.exists()) {
                const detail = { id: docSnap.id, ...docSnap.data() } as IImagePattern;
                return detail;
            }
            return null;
        } finally {
